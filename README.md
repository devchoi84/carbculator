...
| 🔍 **음식 검색 (식약처 DB)** | 음식명을 검색해 여러 음식을 담고, 각 음식의 **섭취량(g)** 을 조절하면 탄수화물이 자동 합산 (식약처 식품영양성분 API) |
| 🕗 **계산 내역 (7일)** | 완료된 식사·인슐린 계산을 최근 7일간 보관하고 결과 화면에서 목록으로 확인 (이후 자동 삭제) |
| 🩺 **실행 시 참고용 안내** | 앱 실행 시 "의료진 지침 우선" 안내 팝업 표시 (다시 보지 않기 지원) |
| 🔒 **로컬 우선 저장** | 설정·계산 내역은 `@AppStorage`(UserDefaults)로 기기에만 저장 · 계정 없음 (음식 검색 시에만 검색어가 외부 API로 전송) |
